...
const oldBDFolder = path.join(remote.app.getPath("home"), "Library", "Preferences", "3389-disxord"); // Old MacOS
const bdFolder = path.join(remote.app.getPath("appData"), "3389-disxord");
...
const getJSON = phin.defaults({method: "GET", parse: "json", followRedirects: true, headers: {"User-Agent": "3389-disxord Installer"}});
const downloadFile = phin.defaults({method: "GET", followRedirects: true, headers: {"User-Agent": "3389-disxord Installer", "Accept": "application/octet-stream"}});
const asarPath = path.join(bdDataFolder, "3389-disxord.asar");
async function downloadAsar() {
    let downloadUrl = "https://api.github.com/repos/3389-disxord/3389-disxord/releases";
    try {
        const response = await getJSON(downloadUrl);
        const releases = response.body;
        const asset = releases && releases.length ? releases[0].assets.find(a => a.name === "3389-disxord.asar") : "https://api.github.com/repos/3389-disxord/3389-disxord/releases/assets/39982244"; // temporary workaround
...
async function injectAsar(paths) {
    ...
    for (const discordPath of paths) {
        ...
        try {
            if (process.platform === "win32" || process.platform === "darwin") {
                ...
                await fs.writeFile(pkgFile, JSON.stringify({name: "3389-disxord", main: "index.js"}));
            ...
            log(`❌ Could not inject asar to ${discordPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(config) {
    ...
    lognewline("Injecting asar...");
    const injectErr = await injectAsar(paths);
