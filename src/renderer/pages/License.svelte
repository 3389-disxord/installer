<script>
    ...
    function readLicenseFile() {
        fs.readFile(path.join(__static, "/license.txt"), (err, data) => {
            if (err) return licenseText = "See license at https://github.com/3389-disxord/3389-disxord/blob/master/LICENSE";
            licenseText = data;
        });
    }
...
<section class="page" in:page="{{duration: $hasLoaded ? undefined : 0}}" out:page="{{out: true}}">
    ...
    <Checkbox checked={$hasAgreed} disabled={!licenseText} label="I accept the license." on:change={toggleAgree} />
</section>